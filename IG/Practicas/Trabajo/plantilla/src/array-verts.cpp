...
void ArrayVertices::visualizarGL_MI_BVE( const GLenum tipo_primitiva )
{
  ...
  GLuint nv = (indices != nullptr)? indices->num_vals_tupla : num_vertices;
  /*  GLuint nv;
   if (indices != nullptr)
    nv = indices->num_vals_tupla;
   else
    nv=num_vertices;
*/
   ...
   {
     GLuint iv= (indices->datos != nullptr)? ((const GLuint *)indices->datos)[i] : i;
     if (colores != nullptr)
